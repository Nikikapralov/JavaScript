const { expect } = require("chai");

const movieTheater = {
    ageRestrictions(movieRating) {
      switch (movieRating) {
        case "G":
          return "All ages admitted to watch the movie";
        case "PG":
          return "Parental guidance suggested! Some material may not be suitable for pre-teenagers";
        case "R":
          return "Restricted! Under 17 requires accompanying parent or adult guardian";
        case "NC-17":
          return "No one under 17 admitted to watch the movie";
        default:
          return "There are no age restrictions for this movie";
      }
    },
    moneySpent(tickets, food, drinks) {
      if (
        typeof tickets !== "number" ||
        !Array.isArray(food) ||
        !Array.isArray(drinks)
      ) {
        throw new Error("Invalid input");
      }
  
      let bill = 0;
  
      bill += tickets * 15;
  
      food.forEach((element) => {
        switch (element) {
          case "Nachos":
            bill += 6;
            break;
          case "Popcorn":
            bill += 4.5;
            break;
        }
      });
  
      drinks.forEach((element) => {
        switch (element) {
          case "Soda":
            bill += 2.5;
            break;
          case "Water":
            bill += 1.5;
            break;
        }
      });
  
      if (bill > 50) {
        bill -= bill * 0.2;
        return `The total cost for the purchase with applied discount is ${bill.toFixed(
          2
        )}`;
      } else {
        return `The total cost for the purchase is ${bill.toFixed(2)}`;
      }
    },
    reservation(rowsArray, neededSeatsCount) {
      if (!Array.isArray(rowsArray) || typeof neededSeatsCount !== "number") {
        throw new Error("Invalid input");
      }
  
      let availableRows = [];
      rowsArray.forEach((row) => {
        if (row.freeSeats >= neededSeatsCount) {
          availableRows.push(row.rowNumber);
        }
      });
  
      return Math.max(...availableRows);
    },
  };
  
  
  describe("Tests …", function() {
      describe("Test ageRestrictions function", function() {
          it("MovieRating is equal to G", function() {
            expect(movieTheater.ageRestrictions("G")).to.be.equal("All ages admitted to watch the movie")
          });
          it("MovieRating is equal to PG", function() {
            expect(movieTheater.ageRestrictions("PG")).to.be.equal("Parental guidance suggested! Some material may not be suitable for pre-teenagers")
          });
          it("MovieRating is equal to R", function() {
            expect(movieTheater.ageRestrictions("R")).to.be.equal("Restricted! Under 17 requires accompanying parent or adult guardian")
          });
          it("MovieRating is equal to NC-17", function() {
            expect(movieTheater.ageRestrictions("NC-17")).to.be.equal("No one under 17 admitted to watch the movie")
          });
          it("MovieRating is equal to else", function() {
            expect(movieTheater.ageRestrictions("else")).to.be.equal("There are no age restrictions for this movie")
          });
       });

    describe("Test moneySpent function", function(){
        it("Test with discount 20% bigger than 50 leva", function(){
            expect(movieTheater.moneySpent(10, [], [])).to.be.equal("The total cost for the purchase with applied discount is 120.00")
        })
        it("Test with no discount", function(){
            expect(movieTheater.moneySpent(1, [], [])).to.be.equal("The total cost for the purchase is 15.00")
        })
        it("Test invalid input", function(){
            expect(() => {movieTheater.moneySpent("dun", "dun", "dun")}).to.throw("Invalid input")
        })
    })

    describe("Test reservation function", function(){
        it("Test wrong input raises an error", function(){
            expect(() => {movieTheater.reservation("not", "err")}).to.throw("Invalid input")
        })
        it("Test returns correct available rows number", function(){
            expect(movieTheater.reservation([{ "rowNumber": 1, "freeSeats": 7 }, { "rowNumber": 2, "freeSeats": 5 }], 1)).to.be.equal(2)
        })
    })
  });
  